...
def denormalize(image, mean_param=[0.485, 0.456, 0.406], std_param=[0.229, 0.224, 0.225]):
...
def visualize_samples(dataloader_train):
  ...
  for i, (inputs, classes) in enumerate(dataloader_train):
    ...
    for j in range(len(classes)):
      ...
      if label not in classes_sampled:
        ...
        img = denormalize(inputs[j]) # Use inputs[j] to get the image corresponding to the label
